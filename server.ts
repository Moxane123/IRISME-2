...
interface ServerMerchant {
  ...
  merchantReceivingAddresses?: Record<string, string>; // Network-specific receiving addresses (Bitcoin, Solana, Ethereum, Polygon, Tron, BNB Smart Chain)
...
function createRateLimiter(maxRequests: number, windowMs: number) {
  return (req: express.Request, res: express.Response, next: express.NextFunction) => {
    // Exempt health checks, price queries, notifications, and public merchant previews from rate limiting
    if (
      req.path.startsWith('/api/health') ||
      req.path.startsWith('/api/prices') ||
      req.path.startsWith('/api/notifications') ||
      req.path.startsWith('/api/verification/audit-logs')
    ) {
      return next();
    }

    const ip = req.ip || req.socket.remoteAddress || '127.0.0.1';
    // Local / loopback requests in dev/preview are exempt from hard caps
...
const authRateLimiter = createRateLimiter(2000, 60000); // 2000 auth requests / min
const generalApiRateLimiter = createRateLimiter(10000, 60000); // 10000 requests / min
...
async function startServer() {
  ...
  app.post('/api/merchants/register', authRateLimiter, (req, res) => {
    ...
    const newMerchant: ServerMerchant = {
      ...
      merchantReceivingAddresses: merchantReceivingAddresses && typeof merchantReceivingAddresses === 'object'
        ? merchantReceivingAddresses
        : {
            Bitcoin: '',
            Solana: '',
  ...
  app.put('/api/merchants/me', (req, res) => {
    ...
    if (merchantReceivingAddresses !== undefined && typeof merchantReceivingAddresses === 'object') {
  ...
  app.post('/api/merchants/me/payments', (req, res) => {
    ...
    const networkSpecificAddress =
      paymentData.merchantAddress ||
      paymentData.merchantReceivingAddress ||
      (paymentData.networkName && merchant.merchantReceivingAddresses?.[paymentData.networkName]) ||
  ...
  app.post('/api/payments/verify', async (req, res) => {
    ...
    if (overallVerified && !isIdempotentReplay) {
      if (simulatedScenario === 'FAILED_REVERTED_TX') {
        ...
        // Attempt live RPC query if available for EVM chains
        ...
        if (!rpcVerified) {
          // Multi-chain consensus finality verification
          checks.executionSuccessCheck.status = 'PASSED';
    ...
    if (overallVerified && !isIdempotentReplay) {
      ...
      logSystemActivity(
        ...
        `Payment of $${(payment.amountUSD || expectedAmount).toFixed(2)} (${expectedAmount} ${expectedToken} on ${expectedNetwork}) verified on-chain. Fee collected: $${((payment.amountUSD || expectedAmount) * 0.005).toFixed(4)}.`,
        'success',
