...
import { Copy, Check, QrCode, Smartphone, Code, ShieldCheck, Sparkles } from 'lucide-react';
...
export const PaymentQRCode: React.FC<PaymentQRCodeProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'qr' | 'uri' | 'json'>('qr');
  ...
  const fallbackJson = JSON.stringify(
    {
      ...
      amount: tokenAmount,
      merchantAddress,
      merchantName: merchantName || 'IRISME Merchant',
      description: itemDescription || 'Merchant Checkout',
    },
    null,
    2
  );
  ...
  const matrix = generateMatrix(canonicalJson || activeValue);
  ...
  return (
    <div className="p-4 sm:p-5 rounded-3xl bg-white border border-slate-200 text-center space-y-4 shadow-md relative overflow-hidden">
      ...
      {/* Mode Switcher */}
      <div className="flex items-center justify-center p-1 bg-slate-100 rounded-2xl border border-slate-200 text-[11px] max-w-sm mx-auto">
        <button
          type="button"
          onClick={() => setViewMode('qr')}
          className={`flex-1 py-1 px-2.5 rounded-xl font-bold transition-all flex items-center justify-center gap-1 cursor-pointer ${
            viewMode === 'qr'
      ...
      {viewMode === 'json' ? (
        <div className="text-left bg-slate-900 text-slate-100 p-3.5 rounded-2xl font-mono text-[11px] max-h-56 overflow-y-auto space-y-1 border border-slate-800 shadow-inner">
          <div className="flex justify-between items-center pb-1 mb-1 border-b border-slate-800 text-slate-400 text-[10px]">
            <span>Canonical Multi-Chain Protocol v1 Payload</span>
            <span className="text-purple-400">{tokenSymbol} on {networkName}</span>
          </div>
        ...
        <div className="flex justify-center items-center py-1">
          <div className="relative inline-block p-4 bg-white rounded-3xl shadow-md border-2 border-slate-200">
            ...
            {/* Center Token & Iris Logo overlay */}
